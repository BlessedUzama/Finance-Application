...
export const BudgetCategoryRow: React.FC<{
  ...
  return (
    <div className="group flex flex-col space-y-2 rounded-xl border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/40 p-3.5 transition-all duration-200 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-900/80">
      {/* Top Header */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="text-sm">{budget.icon || '🏷️'}</span>
          <span className="text-xs font-semibold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-300 transition-colors truncate max-w-[140px]">
            {budget.category}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="rounded p-1 text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer text-xs"
      ...
      <div className="flex items-baseline justify-between text-[11px] font-mono">
        <div className="text-slate-500 dark:text-slate-400">
          Spent: <span className="font-semibold text-slate-900 dark:text-white">{formatCurrency(budget.spentAmount)}</span>
        </div>
        <div className="text-slate-500 dark:text-slate-400">
          Cap: <span className="font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(budget.allocatedAmount)}</span>
        </div>
      </div>
...
export const BudgetTracker: React.FC = () => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-5 backdrop-blur-md space-y-4 shadow-sm dark:shadow-xl">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-200 dark:border-slate-800/80 pb-3">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-slate-900 dark:text-white">Monthly Budget Limits</h3>
            <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[10px] font-mono font-semibold text-blue-600 dark:text-blue-400 border border-blue-500/20">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          <div className="rounded-lg bg-slate-100 dark:bg-slate-950 px-3 py-1.5 border border-slate-200 dark:border-slate-800">
            ...
            <span className="text-slate-400 dark:text-slate-500"> / {formatCurrency(totalAllocated)}</span>
            <span className="ml-2 rounded-full bg-blue-500/10 px-2 py-0.5 text-blue-600 dark:text-blue-400 font-sans text-[10px] font-semibold border border-blue-500/20">
              {totalPercentage}%
            </span>
          </div>
          ...
          <button
            ...
            className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 px-2.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          >
            + Category
          </button>
        </div>
            ...
            <div className="w-full max-w-sm rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xl space-y-4 text-slate-900 dark:text-slate-100">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-slate-900 dark:hover:text-white text-xs p-1 cursor-pointer"
                >
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleSaveEdit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                  <button
                    type="button"
                    onClick={() => setEditingBudget(null)}
                    className="rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
