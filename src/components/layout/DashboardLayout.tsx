...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-200 dark:border-slate-800/80 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md">
        <div className="mx-auto flex flex-wrap h-auto min-h-[4rem] max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8 py-2 gap-2">
          ...
          {/* Theme & Currency Switchers & Action Button */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {/* Theme Toggle Button */}
            <button
              onClick={toggleTheme}
              className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 cursor-pointer shadow-sm active:scale-95 transition-transform"
            ...
            >
              {isDarkMode ? (
                ...
                <svg className="h-4 w-4 text-slate-700 fill-current" viewBox="0 0 20 20">
            ...
            {/* Currency Selector */}
            <div className="flex items-center gap-1 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2 py-1 text-xs font-medium text-slate-800 dark:text-slate-300">
              ...
              <select
                ...
                className="bg-transparent text-slate-900 dark:text-white font-mono font-semibold focus:outline-none cursor-pointer text-xs"
              >
      ...
      <main className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-6 space-y-6 sm:space-y-8">
          ...
          <div className="w-full md:w-80 relative">
            <div className="relative w-full">
              ...
              {searchQuery && isSearchFocused && (
                <div className="absolute right-0 top-full mt-2 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-2xl z-50 space-y-2">
                  <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                    <span>Instant Results ({filteredTransactions.length})</span>
                    <button
                      onClick={() => setIsSearchFocused(false)}
                      className="text-slate-400 hover:text-slate-900 dark:hover:text-white text-[10px] cursor-pointer"
                    >
                      Close ✕
                    </button>
                  </div>
                  ...
                  <div className="max-h-60 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                    {filteredTransactions.length === 0 ? (
                      <p className="py-4 text-center text-slate-400 text-xs">No transactions match "{searchQuery}"</p>
                    ) : (
                      filteredTransactions.map((tx) => (
                        <div
                          key={tx.id}
                          onClick={handleResultClick}
                          className="flex items-center justify-between py-2 px-2 hover:bg-slate-100 dark:hover:bg-slate-800/60 rounded-md cursor-pointer transition-colors"
                        >
                          <div>
                            <span className="font-semibold text-slate-900 dark:text-white block">{tx.merchant}</span>
                          ...
                          <span
                            className={`font-mono font-semibold text-xs ${
                              tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-slate-200'
                            }`}
                          >
        ...
        {searchQuery && (
          <div className="flex items-center justify-between rounded-lg border border-blue-500/20 bg-blue-500/10 px-4 py-2 text-xs text-blue-600 dark:text-blue-300">
            <span>
              Filtering transactions matching: <strong className="text-slate-900 dark:text-white">"{searchQuery}"</strong>
            </span>
            <button
              onClick={() => setSearchQuery('')}
